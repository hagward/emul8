        ...
        switch (opCode & 0xF000) {
            ...
            case 0xF000:
                switch (opCode & 0x00FF) {
                    ...
                    case 0x0033:
                    	memory.setByte(index,     reg[x] / 100);
                    	memory.setByte(index + 1, (reg[x] / 10) % 100);
                    	memory.setByte(index + 2, reg[x] % 100);
                        break;

                    // FX55: Stores V0 to VX in memory starting at address I.
                    case 0x0055:
                        for (int i = 0; i < x; i++) {
                        	memory.setByte(index + i, reg[i]);
                        }
                        // When the operation is done, index should be set to index + x + 1.
                        index += x + 1;
                        break;
